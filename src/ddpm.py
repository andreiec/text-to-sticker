...
class DDPMSampler(nn.Module):
    ...
    def _get_variance(self, t: int, t_prev: int, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
        ...
        alpha_prev = (self.alpha_cumprod[t_prev] if t_prev >= 0 else self.one_scalar.to(device=device, dtype=dtype))
    ...
    def step(self, latents, model_output, t: int, idx: int) -> torch.Tensor:
        ...
        alpha_bar_prev = (self.alpha_cumprod[t_prev] if t_prev >= 0 else torch.tensor(1.0, device=device, dtype=dtype))

        recip_sqrt_alpha = alpha_t.rsqrt()
    ...
    def add_noise(self, original_samples: torch.Tensor, timesteps: torch.Tensor, noise: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        sqrt_alpha = self.sqrt_alpha_cumprod[ts].view((original_samples.shape[0],) + (1,) * (original_samples.ndim - 1))
        sqrt_1m_alpha = self.sqrt_one_minus_alpha_cumprod[ts].view((original_samples.shape[0],) + (1,) * (original_samples.ndim - 1))
